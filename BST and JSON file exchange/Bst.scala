...
sealed abstract class BST[A <% Ordered[A]] extends AbstractSet[A] 
{
  ...
  def containHelper(x:A, b:BST[A]): Boolean = {
    b match {
	case Leaf() => false
	case Node(e,l,r) => 
		if(x == e) true
		else if(x < e) containHelper(x, l)
		else containHelper(x,r)
	}
  }

  def contains(x: A): Boolean = containHelper(x, this)
 ...
 def fold[B](f: (B, A) => B, acc: B): B = {
  ...
  def add(x:A): BST[A] = {
	...
	case Node(y, l, r) => 
		if(y == x) Node(x, l ,r)
		else if( x< y) Node(y, l.add(x), r)
		else Node(y, l , r.add(x))
    }
  }
  ...
  def removeMin : (A, BST[A]) = {
    this match {
	case Leaf() => sys.error("You crazy maan")
	case Node(x, Leaf(), r) => (x, r)
	case Node(x,l,r) => val(remv, tr) = l.removeMin
		(remv, Node(x,tr,r))
    }
  }
...
object BST {
 ...
 def build[A <% Ordered[A]](xs: List[A]): BST[A] = {
	...
	xs.distinct match {  //Check, distinct works
	case y::ys => 
		val left = ys.filter( x=> x <= y)
		val right = ys.filterNot( x=> x <=y)
		Node(y, build(left), build(right))
	case Nil => Leaf()    
   	 }
	}
  }
