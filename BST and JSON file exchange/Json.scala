...
class Doc(val lines: List[String]) { 
  ...
  def hcatT(that: Doc) : Doc = {
    ...
    val dA = Doc.padEnd(that.lines,this.lines.length, "")
    val dB = Doc.padEnd(this.lines, that.lines.length, "")
    for(i<-0 until dA.length) 
       a = (((dB(i) + " "*(this.width-dB(i).length)+ dA(i))::a.reverse).reverse)  
    Doc(a)
  }

  def hcatB(that: Doc) : Doc = {
    var a: List[String] = List()
    val dA = Doc.padBegin(that.lines,this.lines.length, "")
    val dB = Doc.padBegin(this.lines,that.lines.length, "")
    for(i<-0 until dA.length)
      a = (((dB(i) + " "*(this.width-dB(i).length)+dA(i))::a.reverse).reverse)
  ...
  def vcat(that: Doc) : Doc = {
    val a = Doc(this.lines ++ that.lines)
    a 
...
object Doc {
  ...
  def padEnd[A](xs: List[A], n: Int, x:A): List[A] = {
    val diff = n-xs.length
    if(diff>0)
        padEnd(((x :: xs.reverse).reverse), n, x) 
    else
      xs
  }
...
object JVal {
  ...
  def render(jv: JVal) : Doc = {
    jv match {
      ...
      case JObj(o) => renderDocs(o.map((m: (String,JVal)) => 
          (Doc("%s : ".format(m._1)).hcatT(render(m._2)))).toList)
      case JArr(a) => renderDocs(a.map((m: JVal) => (render(m))))
...
object Json { 
  ...
  implicit def tuple3Json[A1 : Json, A2 : Json, A3 : Json] = new Json[(A1, A2, A3)] {
    def json(t: (A1, A2, A3)): JVal = JObj(Map("fst" -> Json.toJson(t._1), "snd" -> Json.toJson(t._2), "thd" -> Json.toJson(t._3)))
  }

  implicit def listJson[A : Json] = new Json[List[A]]{ 
    def json(t: List[A]): JVal = JArr(t.map(Json.toJson(_)))
  }

  implicit def arrJson[A : Json] = new Json[Array[A]] { 
    def json(t: Array[A]): JVal = JArr(t.toList.map(Json.toJson(_)))
  }

  implicit def mapJson[A : Json] = new Json[Map[String, A]] { 
    def json(t: Map[String, A]): JVal = JObj(t.map((m: (String, A)) => 
                        m._1 -> Json.toJson(m._2)))
